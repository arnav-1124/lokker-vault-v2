...
import { decryptVaultEnvelope } from '../vault/secure-storage.js';
  ...
  try {
    if (chrome.storage?.session) {
      ...
      if (sess && sess.isUnlocked && sess.decryptedVault) {
        ...
        debugLog('Successfully rehydrated vault from session storage. Items count:', activeDecryptedVault.length);
        return activeDecryptedVault;
      }
    }
  } catch (e) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (action === 'GET_LOCK_STATUS') {
    chrome.storage.local.get(['vaultMeta', 'encryptedVault'], (storageRes) => {
      const hasVault = !!(storageRes.vaultMeta || storageRes.encryptedVault);
      getActiveVault().then((vault) => {
        sendResponse({
          isUnlocked: !!vault,
          hasVaultData: hasVault,
        });
      });
    });
    return true;
  ...
  if (action === 'UNLOCK_VAULT') {
    ...
    chrome.storage.local.get(['vaultMeta', 'encryptedVault'], async (res) => {
      try {
        ...
        const vaultMetaToUse = meta || { encryptedVault: vault };
        const decrypted = await decryptVaultEnvelope(vaultMetaToUse, masterPassword);
