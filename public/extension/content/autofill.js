...
(function () {
  ...
  const autofillModule = {
    getShadowRoot() {
      let container = document.getElementById('lokker-shadow-container') || document.getElementById('xerox-shadow-container');
      if (!container) {
        container = document.createElement('div');
        container.id = 'lokker-shadow-container';
    ...
    makeDraggable(element) {
      ...
      const onMouseDown = (e) => {
        if (e.target.closest('.lokker-autofill-btn') || e.target.closest('.lokker-dismiss-btn') || e.target.closest('.xerox-autofill-btn') || e.target.closest('.xerox-dismiss-btn')) return;
    ...
    attachAutofillBadge(targetInput, onBadgeClick) {
      ...
      let wrapper = shadow.getElementById('lokker-floating-badge') || shadow.getElementById('xerox-floating-badge');

      if (wrapper) {
        const btn = wrapper.querySelector('.lokker-autofill-btn') || wrapper.querySelector('.xerox-autofill-btn');
        if (btn) {
        ...
        const dismissBtn = wrapper.querySelector('.lokker-dismiss-btn') || wrapper.querySelector('.xerox-dismiss-btn');
        if (dismissBtn) {
      ...
      wrapper.id = 'lokker-floating-badge';
      ...
      const btn = wrapper.querySelector('.lokker-autofill-btn');
      if (btn) {
      ...
      const dismissBtn = wrapper.querySelector('.lokker-dismiss-btn');
      if (dismissBtn) {
      ...
      wrapper.addEventListener('click', (e) => {
        if (e.target.closest('.lokker-autofill-btn') || e.target.closest('.lokker-dismiss-btn')) return;
        if (!dragMoved) {
          onBadgeClick();
        }
      });
    ...
    hideBadge() {
      const container = document.getElementById('lokker-shadow-container') || document.getElementById('xerox-shadow-container');
      if (container && container.shadowRoot) {
        const badge = container.shadowRoot.getElementById('lokker-floating-badge') || container.shadowRoot.getElementById('xerox-floating-badge');
        if (badge) {
  ...
  window.LokkerAutofill = autofillModule;
  window.XeroxAutofill = autofillModule;
})();
