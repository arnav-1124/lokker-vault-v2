/**
 * Login field & form detection module for Lokker Extension
...
(function () {
      ...
      if (passwordInput) {
        ...
        if (!usernameCandidate) {
          const preceding = candidates.filter(
            (i) => (i.compareDocumentPosition(passwordInput) & Node.DOCUMENT_POSITION_PRECEDING) !== 0
          );
          usernameCandidate =
            preceding.slice().reverse().find((i) => {
    ...
    observeDynamicForms(callback) {
      ...
      const observer = new MutationObserver((mutations) => {
        ...
        if (relevant) {
          ...
          timer = setTimeout(() => {
            const fields = detector.findLoginFields();
  ...
  window.LokkerFieldDetector = detector;
  window.XeroxFieldDetector = detector;
})();
